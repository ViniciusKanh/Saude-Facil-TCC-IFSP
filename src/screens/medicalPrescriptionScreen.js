...
const MedicalPrescriptionScreen = () => {
  ...
  useEffect(() => {
    // Função assíncrona para buscar os tipos de prescrição do banco de dados
    async function fetchTypesPrescription() {
      try {
        const querySnapshot = await getDocs(collection(db, "TypePrescription"));
        const fetchedTypes = querySnapshot.docs.map((doc) => ({
          label: doc.data().Type, // Supondo que o campo se chama "Type" no Firestore
          value: doc.id, // Utilizando o ID do documento como valor
        }));
        setTypesPrescription(fetchedTypes);
        console.log("Tipos de Prescrição carregados: ", fetchedTypes); // Para acompanhamento
      } catch (error) {
    ...
    // Função assíncrona para buscar as prescrições médicas do banco de dados
    async function fetchPrescriptions() {
      try {
        const querySnapshot = await getDocs(
          collection(db, "medicalPrescription")
        );
        ...
        console.log("Prescrições Médicas carregadas: ", fetchedPrescriptions); // Para acompanhamento
      } catch (error) {
