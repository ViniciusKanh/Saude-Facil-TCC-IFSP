...
import {
  ...
  deleteDoc,
  doc,
  updateDoc // Deve vir do 'firebase/firestore' e não do 'firebase/storage'
...
const MedicationScreen = () => {
 ...
 useEffect(() => {
  ...
  const handleSaveMedication = async () => {
    ...
    if (typeof selectedImage === 'string' && !selectedImage.startsWith('https://')) {
      // A imagem é nova e precisa ser carregada
      const response = await fetch(selectedImage);
      const blob = await response.blob();
      const fileRef = ref(storage, `Medication_Images/${new Date().toISOString()}`);
  ...
  const fetchAndSetTypes = async () => {
    ...
    try {
      ...
      setTypesConcentration(Object.entries(typeConcentrationMap).map(([key, value]) => ({ label: value, value: key })));
      setTypesDosage(Object.entries(typesDosageMap).map(([key, value]) => ({ label: value, value: key })));
      setTypesMedicines(Object.entries(typesMedicinesMap).map(([key, value]) => ({ label: value, value: key })));
    } catch (error) {
